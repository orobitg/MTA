...
int mta_program_repeated_trees(Parameters *P, Sequence *S, Distance_matrix *DM){
    ...
    if(P->only_tree == 0){
        ...
        if(strm(P->align_method, "t_coffee")){
            libname = (char *) vcalloc(FILENAMELEN, sizeof(char));
            sprintf(libname, "%s%s.lib", P->workdir, (P->F)->name);
        }
...
void master_generate_mta(Parameters *P, Sequence *S, Distance_matrix *DM){
    ...
    int *dest;
    double score, max_score=-9999999.99999;  
    double *sc_list;
    ...
    else if(strm(P->align_method, "clustalo") || strm(P->align_method, "mafft")){
        tmpname = (char *) vcalloc(FILENAMELEN, sizeof(char));
        sprintf(tmpname, "./outtree");
        fptmp = openfile(tmpname, "w");
        fclose(fptmp);
    ...
    for(i = 0; i < P->ntree; i++){
        ...
        if(dest[0]){          
            ...
            MPI_Recv(sc_list, 2, MPI_DOUBLE, MPI_ANY_SOURCE, TAG_WSSCORE, MPI_COMM_WORLD, &status);
            tree = (int) sc_list[0];
            score = (double) sc_list[1];

            sprintf(treename, "%s%s_%d.dnd", P->workdir, (P->F)->name, tree);
            sprintf(alnname, "%s%s_%d.fasta_aln", P->workdir, (P->F)->name, tree);
 
            fps = fopen(scoresfile, "a");
            fprintf(fps, "%d;%s_%d;%lf\n", tree, (P->F)->name, tree, score);
            fclose(fps);
            fprintf(stdout, "\t- %d - Tree: %s_%d.dnd - Aln: %s_%d.fasta_aln  - Score: %lf\n", tree, (P->F)->name, tree, (P->F)->name, tree, score);
            if(score > max_score){
                max_score = score;
                max_ntree=tree;
                /*
...
void worker_generate_mta(Parameters *P, Sequence *S, Distance_matrix *DM){
    ...
    while(tree != FIN_MSG){
        ...
        if(tree != FIN_MSG){
            ...
            MPI_Isend(sc_list, 2, MPI_DOUBLE, 0, TAG_WSSCORE, MPI_COMM_WORLD, &req);   
        ...
        if((P->PB)->tcoffee_bin != NULL){
            if(P->mpi_mode == 0 && libname != NULL){
                ...
                if(P->ntree >= np){
                    ...
                    sprintf(align_app, "%s %s -usetree %s -output=fasta -n_core=1 -outfile=%s > /dev/null 2>&1", (P->PB)->tcoffee_bin, (P->F)->full, treename, alnname);
                }
            }
